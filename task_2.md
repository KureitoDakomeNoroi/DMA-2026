...
### 1. Определение наклона (ориентации) экрана.
...
public class SettingsModel {
    ...
    public void setVolumeLevel(int volumeLevel) {
        if (volumeLevel < 0 || volumeLevel > 100) {
            throw new IllegalArgumentException("Громкость должна быть от 0 до 100, 105 нету. . .");
        }
...
public class GeoPoint {
    ...
    public GeoPoint(double latitude, double longitude) {
        if (latitude < -90 || latitude > 90) {
            throw new IllegalArgumentException("Некорректная широта, проверьте запрос");
        }
        if (longitude < -180 || longitude > 180) {
            throw new IllegalArgumentException("Некорректная долгота, проверьте запрос");
        }
...
public abstract class BaseScreen {
    public void onOpen() {
        System.out.println("Ваш экран открыт!");
    }

    public void onClose() {
        System.out.println("Ваш экран закрыт!");
    }
}
...
public class SyncQueue {
    public static void main(String[] args) {
        ...
        while (!queue.isEmpty()) {
            System.out.println("Обработочка: " + queue.poll());
        }
    }
}
```
...
**3. Генератор одноразового SMS-кода для сообщений.**
...
**5. Маскировка персональных данных почты.**
...
**7. Шифратор перестановочный для локальных заметок.**
...
**13. Группировка контактов по первой букве алфавита.**
...
**16. Подсчет суммарной стоимости корзины заказа.**
